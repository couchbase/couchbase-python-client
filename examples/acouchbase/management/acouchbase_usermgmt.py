...
async def main():
    ...
    adm_cluster = Cluster(
        ...
        authenticator=PasswordAuthenticator(
            "Administrator",
            "password"))
    await adm_cluster.on_connect()
    # For Server versions 6.5 or later you do not need to open a bucket here
    adm_bucket = adm_cluster.bucket(bucket_name)
    await adm_bucket.on_connect()
    ...
    await user_manager.upsert_user(user)

    users_metadata = await user_manager.get_all_users()
    ...
    await collection.upsert("airline_11", airline_11)
