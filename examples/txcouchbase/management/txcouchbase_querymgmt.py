...
def main():
    ...
    try:
        ...
        yield ixm.create_primary_index(bucket_name)
    except QueryIndexAlreadyExistsException:
        print('Primary index already exists')

    # can create an index, and ignore if it already exists
    yield ixm.create_primary_index(bucket_name, CreatePrimaryQueryIndexOptions(ignore_if_exists=True))
    ...
    fields = ['type']
    yield ixm.create_index(bucket_name,
    ...
    indexes = yield ixm.get_all_indexes(bucket_name)
    ...
    yield ixm.build_deferred_indexes(bucket_name)

    # lets wait until the indexes are built
    # @TODO: txcouchbase watch indexes
    # wait_seconds = 30
    # for _ in range(3):
    #     try:
    #         yield ixm.watch_indexes(bucket_name,
    #                             ix_names,
    ...
    # now lets do query!
    query_res = yield cluster.query(n1ql)
    for row in query_res.rows():
        print('Query row: {}'.format(row))

    # time to drop the indexes
    yield ixm.drop_index(bucket_name, beer_sample_type_idx)
    yield ixm.drop_index(bucket_name, beer_sample_brewery_id_idx)
    ...
    yield ixm.drop_index(bucket_name, beer_sample_brewery_id_idx, DropQueryIndexOptions(ignore_if_not_exists=True))
