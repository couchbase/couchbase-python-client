...
get_mutation_state(PyObject* pyObj_mutation_state)
{
  ...
  for (size_t ii = 0; ii < ntokens; ++ii) {
    ...
    auto token =
      couchbase::mutation_token{ partition_uuid, sequence_number, partition_id, bucket_name };
...
build_query_request(PyObject* pyObj_query_args)
{
  ...
  if (pyObj_scan_consistency != nullptr) {
    if (PyUnicode_Check(pyObj_scan_consistency)) {
      req.scan_consistency = str_to_scan_consistency_type<couchbase::query_scan_consistency>(
      ...
      if (PyBytes_Check(pyOb_param)) {
        try {
          ...
          PyErr_SetString(PyExc_ValueError,
                          "Unable to parse positional paramter option value. Positional parameter "
