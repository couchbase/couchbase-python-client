...
template<>
result*
add_extras_to_result<couchbase::core::operations::increment_response>(
  const couchbase::core::operations::increment_response& resp,
  result* res)
{
...
template<>
result*
add_extras_to_result<couchbase::core::operations::decrement_response>(
  const couchbase::core::operations::decrement_response& resp,
  result* res)
{
  PyObject* pyObj_tmp = PyLong_FromUnsignedLongLong(resp.content);
  if (-1 == PyDict_SetItemString(res->dict, "content", pyObj_tmp)) {
                                      ...
                                      result* multi_result = nullptr)
{
  ...
  if (resp.ctx.ec()) {
    pyObj_exc =
      build_exception_from_context(resp.ctx, __FILE__, __LINE__, "Binary operation error.");
  ...
  if (set_exception) {
    pyObj_exc = pycbc_build_exception(
      PycbcError::UnableToBuildResult, __FILE__, __LINE__, "Binary operation error.");
    if (pyObj_errback == nullptr) {
      if (multi_result != nullptr) {
        Py_INCREF(Py_False);
        barrier->set_value(Py_False);
  ...
  if (!PyBytes_Check(options->pyObj_value)) {
    if (multi_result != nullptr) {
      PyObject* pyObj_exc = pycbc_build_exception(
        PycbcError::InvalidArgument, __FILE__, __LINE__, "Value should be bytes object.");
...
handle_binary_op([[maybe_unused]] PyObject* self, PyObject* args, PyObject* kwargs)
{
  ...
  if (!ret) {
    pycbc_set_python_exception(PycbcError::InvalidArgument,
                               __FILE__,
                               __LINE__,
                               "Cannot perform binary operation.  Unable to parse args/kwargs.");
    return nullptr;
  }
  ...
  switch (op_type) {
    ...
    case Operations::PREPEND: {
      ...
      try {
        pyObj_op_response =
          prepare_and_execute_binary_mutation_op(&opts, pyObj_callback, pyObj_errback, barrier);
    ...
    case Operations::DECREMENT: {
      ...
      try {
        pyObj_op_response =
          prepare_and_execute_counter_op(&opts, pyObj_callback, pyObj_errback, barrier);
      } catch (const std::system_error& e) {
        barrier->set_value(nullptr);
        pycbc_set_python_exception(e.code(), __FILE__, __LINE__, e.what());
        Py_XDECREF(pyObj_callback);
    ...
    default: {
      pycbc_set_python_exception(PycbcError::InvalidArgument,
                                 __FILE__,
                                 __LINE__,
                                 "Unrecognized binary operation passed in.");
      barrier->set_value(nullptr);
      Py_XDECREF(pyObj_callback);
      Py_XDECREF(pyObj_errback);
      break;
      ...
      if (PyDict_Check(pyObj_op_dict) && !k.empty()) {
        ...
        switch (op_type) {
          ...
          case Operations::PREPEND: {
            ...
            try {
              pyObj_op_response = prepare_and_execute_binary_mutation_op(
                &opts, nullptr, nullptr, barrier, multi_result);
            } catch (const std::system_error& e) {
          ...
          case Operations::DECREMENT: {
            ...
            try {
              pyObj_op_response =
                prepare_and_execute_counter_op(&opts, nullptr, nullptr, barrier, multi_result);
            } catch (const std::system_error& e) {
              PyObject* pyObj_exc = pycbc_build_exception(e.code(), __FILE__, __LINE__, e.what());
              barrier->set_value(pyObj_exc);
            }
