...
template<>
result*
add_extras_to_result<couchbase::core::operations::lookup_in_response>(
...
template<>
result*
add_extras_to_result<couchbase::core::operations::mutate_in_response>(
  const couchbase::core::operations::mutate_in_response& resp,
  result* res)
{
...
template<>
result*
create_base_result_from_subdoc_op_response(
  const char* key,
  const couchbase::core::operations::lookup_in_all_replicas_response::entry& resp)
{
  PyObject* pyObj_result = create_result_obj();
                                      ...
                                      std::shared_ptr<std::promise<PyObject*>> barrier)
{
  ...
  if (resp.ctx.ec().value()) {
    pyObj_exc =
      build_exception_from_context(resp.ctx, __FILE__, __LINE__, "Subdoc operation error.");
  ...
  if (set_exception) {
    pyObj_exc = pycbc_build_exception(
      PycbcError::UnableToBuildResult, __FILE__, __LINE__, "Subdoc operation error.");
    if (pyObj_errback == nullptr) {
      barrier->set_value(pyObj_exc);
    } else {
      pyObj_func = pyObj_errback;
...
template<>
void
create_result_from_subdoc_op_response(
  const char* key,
             ...
             std::shared_ptr<std::promise<PyObject*>> barrier)
{
  ...
  Py_BEGIN_ALLOW_THREADS conn.cluster_.execute(
    req, [key = req.id.key(), pyObj_callback, pyObj_errback, barrier](response_type resp) {
      create_result_from_subdoc_op_response(
        key.c_str(), resp, pyObj_callback, pyObj_errback, barrier);
    });
  ...
  for (ii = 0; ii < nspecs; ++ii) {
    ...
    try {
      ...
      specs.emplace_back(couchbase::core::impl::subdoc::command{
        ...
        new_spec.path,
        {},
        couchbase::core::impl::subdoc::build_lookup_in_path_flags(new_spec.xattr, false) });
    } catch (const std::exception& e) {
      PyErr_SetString(PyExc_ValueError,
                      fmt::format("Invalid subdocument opcode {}", new_spec.op).c_str());
      if (barrier) {
        barrier->set_value(nullptr);
      }
      Py_XDECREF(pyObj_callback);
...
get_lookup_in_options(PyObject* op_args)
{
  ...
  opts.access_deleted =
    pyObj_access_deleted != nullptr && pyObj_access_deleted == Py_True ? true : false;
...
get_mutate_in_options(PyObject* op_args)
{
  ...
  opts.create_as_deleted =
    pyObj_create_as_deleted != nullptr && pyObj_create_as_deleted == Py_True ? true : false;
...
handle_subdoc_op([[maybe_unused]] PyObject* self, PyObject* args, PyObject* kwargs)
{
  ...
  if (!ret) {
    pycbc_set_python_exception(PycbcError::InvalidArgument,
                               __FILE__,
                               __LINE__,
                               "Cannot perform subdoc operation.  Unable to parse args/kwargs.");
    return nullptr;
  }
  ...
  if (!PyTuple_Check(pyObj_spec) && !PyList_Check(pyObj_spec)) {
    pycbc_set_python_exception(PycbcError::InvalidArgument,
                               __FILE__,
                               __LINE__,
                               "Cannot perform subdoc operation.  Value must be a tuple or list.");
    return nullptr;
  }
  ...
  if (nspecs == 0) {
    pycbc_set_python_exception(PycbcError::InvalidArgument,
                               __FILE__,
                               __LINE__,
                               "Cannot perform subdoc operation.  Need at least one command.");
    return nullptr;
  }
  ...
  switch (op_type) {
    ...
    case Operations::LOOKUP_IN_ALL_REPLICAS: {
      ...
      opts.op_type = op_type;
      opts.specs = pyObj_spec;
      prepare_and_execute_lookup_in_all_replicas_op(
        &opts, nspecs, pyObj_callback, pyObj_errback, barrier);
      break;
    }
    case Operations::LOOKUP_IN_ANY_REPLICA: {
      ...
      opts.op_type = op_type;
      opts.specs = pyObj_spec;
      prepare_and_execute_lookup_in_any_replica_op(
        &opts, nspecs, pyObj_callback, pyObj_errback, barrier);
      break;
    }
    ...
    default: {
      pycbc_set_python_exception(PycbcError::InvalidArgument,
                                 __FILE__,
                                 __LINE__,
                                 "Unrecognized subdoc operation passed in.");
      if (barrier) {
        barrier->set_value(nullptr);
      }
      Py_XDECREF(pyObj_callback);
