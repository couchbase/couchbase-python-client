  ...
  if (pyObj_func != nullptr) {
    ...
    if (pyObj_callback_res) {
      ...
      pycbc_set_python_exception(
        PycbcError::InternalSDKError, __FILE__, __LINE__, "Close connection callback failed.");
    }
    Py_DECREF(pyObj_args);
    Py_XDECREF(pyObj_callback);
    Py_XDECREF(pyObj_errback);
                           ...
                           std::shared_ptr<std::promise<PyObject*>> barrier)
{
  ...
  if (pyObj_func != nullptr) {
    ...
    if (pyObj_callback_res) {
      ...
      pycbc_set_python_exception(
        PycbcError::InternalSDKError, __FILE__, __LINE__, "Create connection callback failed.");
    }
    Py_DECREF(pyObj_args);
    Py_XDECREF(pyObj_callback);
    Py_XDECREF(pyObj_errback);
...
get_cluster_credentials(PyObject* pyObj_auth)
{
  ...
  PyObject* pyObj_allowed_sasl_mechanisms =
    PyDict_GetItemString(pyObj_auth, "allowed_sasl_mechanisms");
                               ...
                               PyObject* pyObj_timeout_opts)
{
  ...
  if (pyObj_key_value_timeout != nullptr) {
    auto key_value_timeout =
      static_cast<uint64_t>(PyLong_AsUnsignedLongLong(pyObj_key_value_timeout));
    auto key_value_timeout_ms =
      std::chrono::milliseconds(std::max(0ULL, key_value_timeout / 1000ULL));
    options.key_value_timeout = key_value_timeout_ms;
  }

  PyObject* pyObj_key_value_durable_timeout =
    PyDict_GetItemString(pyObj_timeout_opts, "key_value_durable_timeout");
  if (pyObj_key_value_durable_timeout != nullptr) {
    auto key_value_durable_timeout =
      static_cast<uint64_t>(PyLong_AsUnsignedLongLong(pyObj_key_value_durable_timeout));
    auto key_value_durable_timeout_ms =
      std::chrono::milliseconds(std::max(0ULL, key_value_durable_timeout / 1000ULL));
  ...
  PyObject* pyObj_idle_http_connection_timeout =
    PyDict_GetItemString(pyObj_timeout_opts, "idle_http_connection_timeout");
  if (pyObj_idle_http_connection_timeout != nullptr) {
    auto idle_http_connection_timeout =
      static_cast<uint64_t>(PyLong_AsUnsignedLongLong(pyObj_idle_http_connection_timeout));
    auto idle_http_connection_timeout_ms =
      std::chrono::milliseconds(std::max(0ULL, idle_http_connection_timeout / 1000ULL));
    options.idle_http_connection_timeout = idle_http_connection_timeout_ms;
  }

  PyObject* pyObj_config_idle_redial_timeout =
    PyDict_GetItemString(pyObj_timeout_opts, "config_idle_redial_timeout");
  if (pyObj_config_idle_redial_timeout != nullptr) {
    auto config_idle_redial_timeout =
      static_cast<uint64_t>(PyLong_AsUnsignedLongLong(pyObj_config_idle_redial_timeout));
    auto config_idle_redial_timeout_ms =
      std::chrono::milliseconds(std::max(0ULL, config_idle_redial_timeout / 1000ULL));
                       ...
                       PyObject* pyObj_auth)
{
  ...
  PyObject* pyObj_enable_tcp_keep_alive =
    PyDict_GetItemString(pyObj_options, "enable_tcp_keep_alive");
  ...
  PyObject* pyObj_enable_unordered_execution =
    PyDict_GetItemString(pyObj_options, "enable_unordered_execution");
  ...
  PyObject* pyObj_tcp_keep_alive_interval =
    PyDict_GetItemString(pyObj_options, "tcp_keep_alive_interval");
  if (pyObj_tcp_keep_alive_interval != nullptr) {
    auto tcp_keep_alive_interval =
      static_cast<uint64_t>(PyLong_AsUnsignedLongLong(pyObj_tcp_keep_alive_interval));
    auto tcp_keep_alive_interval_ms =
      std::chrono::milliseconds(std::max(0ULL, tcp_keep_alive_interval / 1000ULL));
  ...
  PyObject* pyObj_max_http_connections =
    PyDict_GetItemString(pyObj_options, "max_http_connections");
  if (pyObj_max_http_connections != nullptr) {
    auto max_http_connections =
      static_cast<size_t>(PyLong_AsUnsignedLong(pyObj_max_http_connections));
...
handle_create_connection([[maybe_unused]] PyObject* self, PyObject* args, PyObject* kwargs)
{
  ...
  if (pyObj_conn == nullptr) {
    pycbc_set_python_exception(PycbcError::InternalSDKError,
                               __FILE__,
                               __LINE__,
  ...
  auto f = barrier->get_future();
  {
    ...
    Py_BEGIN_ALLOW_THREADS conn->cluster_.open(
      ...
      [pyObj_conn, pyObj_callback, pyObj_errback, callback_count, barrier](
