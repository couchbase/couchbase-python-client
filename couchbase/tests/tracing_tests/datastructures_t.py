...
class DataStructuresTracingTestsSuite:
    """Test suite for data structures operation tracing.

    Tests CouchbaseList, CouchbaseMap, CouchbaseQueue, and CouchbaseSet
    operations with tracing. These data structures use sub-operations
    (Get, Insert, MutateIn, LookupIn, Remove) under the hood.
    """
    ...
    def test_kv_ds_list(self, cb_env: KeyValueTracingEnvironment) -> None:
    ...
    def test_kv_ds_map(self, cb_env: KeyValueTracingEnvironment) -> None:
    ...
    def test_kv_ds_queue(self, cb_env: KeyValueTracingEnvironment) -> None:
    ...
    def test_kv_ds_set(self, cb_env: KeyValueTracingEnvironment) -> None:
...
class ClassicDataStructuresTracingTests(DataStructuresTracingTestsSuite):

    @pytest.fixture(scope='class', autouse=True)
    def manifest_validated(self):
        def valid_test_method(meth):
            attr = getattr(ClassicDataStructuresTracingTests, meth)
            return callable(attr) and not meth.startswith('__') and meth.startswith('test')
        method_list = [meth for meth in dir(ClassicDataStructuresTracingTests) if valid_test_method(meth)]
        test_list = set(DataStructuresTracingTestsSuite.TEST_MANIFEST).symmetric_difference(method_list)
        if test_list:
    ...
    def couchbase_test_environment(self, cb_base_env, request):
        # Data structures use KeyValueTracingEnvironment (same as single KV ops)
