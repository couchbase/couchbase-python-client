...
class TransactionTestSuite:
    ...
    def test_binary(self, cb_env):
        ...
        def txn_logic(ctx):
            ctx.insert(cb_env.collection, key, value, TransactionInsertOptions(transcoder=tc))
            get_res = ctx.get(cb_env.collection, key, TransactionGetOptions(transcoder=tc))
            assert get_res.content_as[bytes] == value
            replace_res = ctx.replace(get_res, new_content, TransactionReplaceOptions(transcoder=tc))
        ...
        cb_env.cluster.transactions.run(txn_logic)
        result = cb_env.collection.get(key, transcoder=tc)
        assert result.content_as[bytes] == new_content

    @pytest.mark.usefixtures('check_binary_txns_supported')
    def test_binary_kwargs(self, cb_env):
        key = cb_env.get_new_doc(key_only=True)
        tc = RawBinaryTranscoder()
        value = 'bytes content'.encode('utf-8')
        new_content = b'\xFF'
        ...
        def txn_logic(ctx):
            ctx.insert(cb_env.collection, key, value, transcoder=tc)
            get_res = ctx.get(cb_env.collection, key, transcoder=tc)
            assert get_res.content_as[bytes] == value
            replace_res = ctx.replace(get_res, new_content, transcoder=tc)
            # there's a bug where we don't return the correct content in the replace, so comment this out for now
            # assert replace_res.content_as[bytes] == new_content
            assert get_res.cas != replace_res.cas
    ...
    def test_binary_not_supported(self, cb_env):
        key = cb_env.get_new_doc(key_only=True)
        tc = RawBinaryTranscoder()
        value = 'bytes content'.encode('utf-8')
    ...
    def test_get_multi_binary(self, cb_env):
        ...
        cb_env.collection.insert(key1, value1, transcoder=tc)
        cb_env.collection.insert(key2, value2)
        ...
        for k in [key1, key2]:
            try:
                cb_env.collection.remove(k)
            except DocumentNotFoundException:
                pass

    def test_get_multi_not_exists(self, cb_env):
        key1, value1 = cb_env.get_new_doc()
        key2 = cb_env.get_new_doc(key_only=True)
        cb_env.collection.insert(key1, value1)
        ...
        cb_env.cluster.transactions.run(txn_logic)
        try:
            cb_env.collection.remove(key1)
        except DocumentNotFoundException:
            pass

    def test_get_multi_invalid_index(self, cb_env):
        key1, value1 = cb_env.get_new_doc()
        key2 = cb_env.get_new_doc(key_only=True)
        cb_env.collection.insert(key1, value1)
    ...
    def test_get_multi_options(self, cb_env):
        key1, value1 = cb_env.get_new_doc()
        key2, value2 = cb_env.get_new_doc()
        cb_env.collection.insert(key1, value1)
        cb_env.collection.insert(key2, value2)
    ...
    def test_get_multi_simple(self, cb_env):
        keys_and_docs = []
        for _ in range(3):
    ...
    def test_get_multi_replicas_from_preferred_server_group_binary(self, cb_env):
        ...
        def txn_logic(ctx):
            ...
            res = ctx.get_multi_replicas_from_preferred_server_group(specs)
    ...
    def test_get_multi_replicas_from_preferred_server_group_options(self, cb_env):
        ...
        def txn_logic(ctx):
            ...
            res = ctx.get_multi_replicas_from_preferred_server_group(specs, opts)
            assert isinstance(res, TransactionGetMultiReplicasFromPreferredServerGroupResult)
            assert res.content_as[dict](0) == value1
            assert res.content_as[dict](1) == value2
    ...
    @pytest.mark.usefixtures('check_txn_queries_supported')
    def test_query_mode_insert(self, cb_env):
        coll = cb_env.collection
        key, value = cb_env.get_new_doc()
        key1, value1 = cb_env.get_new_doc()
    ...
    def test_remove_fail_bad_cas(self, cb_env):
        ...
        try:
            cb_env.cluster.transactions.run(txn_logic, TransactionOptions(timeout=timedelta(seconds=2)))
        except Exception as ex:
...
class ClassicTransactionTests(TransactionTestSuite):
    ...
    def couchbase_test_environment(self, cb_base_txn_env, test_manifest_validated, request):
        ...
        cb_base_txn_env.setup(request.param, __name__)
        yield cb_base_txn_env
        cb_base_txn_env.teardown(request.param, __name__)
