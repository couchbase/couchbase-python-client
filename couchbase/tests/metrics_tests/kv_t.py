...
class KeyValueMetricsTestsSuite:
    ...
    def test_kv_mutation_op(self, cb_env: MetricsEnvironment, op_name: OpName) -> None:
    ...
    def test_kv_mutation_op_with_error(self, cb_env: MetricsEnvironment, op_name: OpName) -> None:
        if op_name == OpName.Insert:
            key, value = cb_env.get_existing_doc()
        else:
            key, value = cb_env.get_new_doc()
    ...
    def test_kv_remove_op(self, cb_env: MetricsEnvironment) -> None:
        key, value = cb_env.get_new_doc()
    ...
    def test_kv_remove_op_with_error(self, cb_env: MetricsEnvironment) -> None:
        key = cb_env.get_new_doc(key_only=True)
        validator = cb_env.kv_meter_validator
    ...
    def test_kv_subdoc_op(self, cb_env: MetricsEnvironment, op_name: OpName) -> None:
    ...
    def test_kv_subdoc_op_error(self, cb_env: MetricsEnvironment, op_name: OpName) -> None:
        if op_name in (OpName.LookupInAllReplicas, OpName.LookupInAnyReplica):
            if not EnvironmentFeatures.is_feature_supported('subdoc_replica_read',
                                                            cb_env.server_version_short,
                                                            cb_env.mock_server_type):
    ...
    def test_kv_unlock_op(self, cb_env: MetricsEnvironment) -> None:
        key = cb_env.get_existing_doc(key_only=True)
    ...
    def test_kv_unlock_op_with_error(self, cb_env: MetricsEnvironment) -> None:
        key = cb_env.get_new_doc(key_only=True)
        validator = cb_env.kv_meter_validator
    ...
    def test_kv_upsert_op_with_encoding_error(self, cb_env: MetricsEnvironment) -> None:
        key, value = cb_env.get_new_doc()
        validator = cb_env.kv_meter_validator
