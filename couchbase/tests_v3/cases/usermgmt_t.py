...
class UserManagementTests(CollectionTestCase):
    ...
    def setUp(self, *args, **kwargs):
        ...
        try:
            ...
            self.bm.create_bucket(CreateBucketSettings(name="default", bucket_type="couchbase", ram_quota_mb=100))
    ...
    def validate_user(self, user, user_roles=None):
        ...
        for p in properties:
            ...
            if p == 'username':
                ...
                if len(value) > 0:
                    self.assertTrue(all(map(lambda r: isinstance(r, str), value)))
            elif p == 'roles':
                self.assertIsInstance(value, set)
                if len(value) > 0:
                    self.assertTrue(all(map(lambda r: isinstance(r, Role), value)))
    ...
    def validate_user_and_metadata(self, user_metadata, user_roles=None, groups=None):
        property_list = ['domain', 'user', 'effective_roles', 'password_changed', 'external_groups']
        ...
        for p in properties:
            ...
            if p == 'domain':
                ...
                if len(value) > 0:
                    self.assertTrue(all(map(lambda r: isinstance(r, RoleAndOrigins), value)))
        ...
        if user_roles:
            actual_roles = set(r.role for r in user_metadata.effective_roles 
    ...
    def validate_group(self, group, roles=None):
        property_list = ['name', 'description', 'roles', 'ldap_group_reference']
    ...
    def test_api_object_creation(self):
        ...
        roles = Role('admin')
        user = User(username='test-user', roles=roles, password='test-password')
    ...
    def test_internal_user(self):
        ...
        roles = [Role(name='data_reader', bucket='default'), 
                        Role(name='data_writer', bucket='default')]
        initial_user = User(username=username, 
                                 roles=roles, 
                                 password=password)

        # create user
        self.um.upsert_user(User(username=username, 
                                 roles=roles, 
                                 password=password), 
        ...
        if self.supports_collections():
            test_roles = []
            for r in roles:
                test_roles.append(Role(name=r.name,bucket=r.bucket,scope='*',collection='*'))
        ...
        self.um.upsert_user(user, 
                            UpsertUserOptions(domain_name="local"))

        # get user and verify updates
        user_metadata = self.try_n_times(10, 1, 
                                         self.um.get_user, 
                                         username, 
                                         GetUserOptions(domain_name="local"))
        ...
        self.um.drop_user(username, 
                          DropUserOptions(domain_name="local"))
        self.try_n_times_till_exception(10, 1, self.um.get_user, 
                                        username, 
                                        domain_name="local",
    ...
    def test_external_user(self):
        ...
        # remove user
        self.um.drop_user(username, 
                          DropUserOptions(domain_name="external"))
        self.try_n_times_till_exception(10, 1, self.um.get_user, 
                                        username, 
    ...
    def test_default_domain(self):
        ...
        username = 'custom-user'
        password = 's3cr3t'
        roles = [Role(name='data_reader', bucket='default'), Role(name='data_writer', bucket='default')]

        self.um.upsert_user(User(username=username, password=password, roles=roles))
        ...
        result = all(map(lambda um: self.validate_user_and_metadata(um), users_metadata))
    ...
    def test_invalid_domain_raises_argument_error(self):
        ...
        self.assertRaises(InvalidArgumentException, self.um.get_all_users, domain_name="fake-domain")
        self.assertRaises(InvalidArgumentException, self.um.get_user, username, domain_name="fake-domain")
        self.assertRaises(InvalidArgumentException, self.um.upsert_user, User(username=username, password=password, roles=roles),
                          domain_name="fake-domain")
        self.assertRaises(InvalidArgumentException, self.um.drop_user, username, domain_name="fake-domain")
    ...
    def test_external_nopassword(self):
        ...
        self.assertRaises(InvalidArgumentException, self.um.upsert_user, User(username=username, password=password, roles=roles),
                          domain_name="external")
        self.assertRaises(InvalidArgumentException, self.um.upsert_user, User(username=username, password=password, roles=None),
                          domain_name="external")
        self.assertRaises(InvalidArgumentException, self.um.upsert_user, User(username=username, password=password, roles=[]),
                          domain_name="external")
        try:
            self.um.upsert_user(User(username=username, password=None, roles=roles), 
                                UpsertUserOptions(domain_name="external"))
    ...
    def test_user_scopes_collections(self):
        ...
        for _ in range(3):
            scope = next((s for s in cm.get_all_scopes() if s.name == 'test-scope'), None)
            if scope:
                break
            time.sleep(1)
        ...
        for _ in range(3):
            scope = next((s for s in cm.get_all_scopes() if s.name == 'test-scope'), None)
            if scope:
                coll = next((c for c in scope.collections if c.name == 'test-collection'), None)
                if coll:
                    break
            time.sleep(1)
        ...
        username = 'custom-user'
        password = 's3cr3t'
        roles = [Role(name='data_reader', bucket='default', scope='test-scope'), 
                        Role(name='data_writer', bucket='default', scope='test-scope', collection='test-collection')]
        initial_user = User(username=username, 
                                 roles=roles, 
                                 password=password)
        ...
        if self.supports_collections():
            ...
            for r in roles:
                if not r.collection:
                    test_roles.append(Role(name=r.name,bucket=r.bucket,scope=r.scope,collection='*'))
                else:
                    test_roles.append(r)
    ...
    def test_group_feature_not_found(self):
        ...
        test_group = Group(name='my-test-group', roles=roles, description="test group description")

        self.assertRaises(FeatureNotFoundException, self.um.upsert_group, test_group)
        self.assertRaises(FeatureNotFoundException, self.um.get_all_groups)
        self.assertRaises(FeatureNotFoundException, self.um.get_group, test_group.name)
        self.assertRaises(FeatureNotFoundException, self.um.drop_group, test_group.name)
    ...
    def test_group(self):
        ...
        self.try_n_times_till_exception(10, 1, self.um.get_group, 
                                        test_group.name, 
                                        expected_exceptions=GroupNotFoundException)

    @skip_if_no_groups
    def test_user_and_groups(self):
        user_roles = [Role(name='query_select', bucket='default'), 
                        Role(name='fts_searcher', bucket='default')]
        group_roles = [Role(name='data_reader', bucket='*'), 
                        Role(name='data_writer', bucket='*')]
        groups = [Group(name='my-test-group', roles=group_roles, description="test group description"),
            Group(name='my-test-group-1', roles=Role(name='admin'), description="test group description")]
        ...
        self.assertIsNotNone(user_metadata)
        self.validate_user_and_metadata(user_metadata, user_roles=test_roles, groups=groups)
    ...
    def test_get_all_groups(self):
        roles = [Role(name='data_reader', bucket='*'), 
                        Role(name='data_writer', bucket='*')]
        fresh_group = Group(name='my-test-group', roles=roles, description="test group description")
    ...
    def test_get_roles(self):
        ...
        for rad in reversed(roles):
            ...
            if desc_matches:
                self.assertTrue(rad.role.name == 'admin' and rad.display_name == 'Full Admin')
                return
