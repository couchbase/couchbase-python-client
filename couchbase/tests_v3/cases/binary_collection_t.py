...
class BinaryCollectionTests(CollectionTestCase):
    ...
    def test_append_string_not_empty(self):
        ...
        self.assertIsNotNone(
            self.coll.binary().append(
                ...
                format=FMT_UTF8).cas)
        self.assertEqual(
            "XXXXfoo", self.coll.get(
                self.UTF8_KEY).content_as[str])

    def test_prepend_string_not_empty(self):
        self.coll.upsert(self.UTF8_KEY, "XXXX", format=FMT_UTF8)
        self.assertIsNotNone(
            self.coll.binary().prepend(
                self.UTF8_KEY, "foo").cas)
        self.assertEqual(
            "fooXXXX", self.coll.get(
                self.UTF8_KEY).content_as[str])
    ...
    def test_append_bytes(self):
        self.assertIsNotNone(
            self.coll.binary().append(
                self.BYTES_KEY,
                b'XXX',
                format=FMT_BYTES).cas)
        self.assertEqual(
            b'XXX', self.coll.get(
                self.BYTES_KEY).content_as[bytes])
    ...
    def test_append_bytes_not_empty(self):
        ...
        self.assertIsNotNone(
            self.coll.binary().append(
                ...
                format=FMT_BYTES).cas)
        self.assertEqual(
            b'XXXfoo', self.coll.get(
                self.BYTES_KEY).content_as[bytes])

    def test_counter_increment_default_no_key(self):
        result = self.coll.binary().increment(
    ...
    def test_counter_decrement_default_no_key(self):
        result = self.coll.binary().decrement(
    ...
    def test_signed_int_64(self):
        self.assertRaises(InvalidArgumentException,
                          SignedInt64, -0x7FFFFFFFFFFFFFFF - 2)
        self.assertRaises(
            InvalidArgumentException,
            SignedInt64,
            0x7FFFFFFFFFFFFFFF + 1)
