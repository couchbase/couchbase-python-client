...
class CollectionManagementTests:
    ...
    def test_create_collection_kwargs(self, cb_env):
        ...
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                              scope_name='_default',
                              collection_name=self.TEST_COLLECTION)
        assert cb_env.get_collection('_default', self.TEST_COLLECTION) is not None

    @pytest.mark.usefixtures("cleanup_collection")
    def test_create_collection_mixed_kwargs(self, cb_env):
    ...
    @pytest.mark.usefixtures("cleanup_collection")
    def test_create_collection_settings_kwarg(self, cb_env):
        ...
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                              ...
                              settings=settings)
        coll_spec = cb_env.get_collection('_default', self.TEST_COLLECTION)
    ...
    def test_create_collection_with_options(self, cb_env):
        ...
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                              '_default', self.TEST_COLLECTION, None, opts)
        assert cb_env.get_collection('_default', self.TEST_COLLECTION) is not None

    @pytest.mark.usefixtures("cleanup_collection")
    def test_create_collection_timeout_kwarg(self, cb_env):
    ...
    def test_create_collection_collection_spec_kwarg(self, cb_env):
        ...
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, collection=spec)
        assert cb_env.get_collection('_default', self.TEST_COLLECTION) is not None

    def test_create_collection_invalid_args(self, cb_env):
        spec = CollectionSpec('coll', 'scope')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, spec, collection=spec)
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, spec, scope_name='_default')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                                  spec, settings=CreateCollectionSettings())
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                                  collection=spec, collection_name='c')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, '_default', collection=spec)
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, '_default', scope_name='_default')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                                  '_default', 'c', collection_name='c')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                                  '_default', 'c', CreateCollectionSettings(),
                                  settings=CreateCollectionSettings())
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.create_collection,
                                  '_default', 'c', settings='not-a-settings')
    ...
    def test_drop_collection_kwargs(self, cb_env):
        ...
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, scope_name, collection_name)
        assert cb_env.get_collection(scope_name, collection_name) is not None
        run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection,
    ...
    def test_drop_collection_with_options(self, cb_env):
        ...
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, scope_name, collection_name)
        assert cb_env.get_collection(scope_name, collection_name) is not None
        run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, scope_name, collection_name, opts)

    @pytest.mark.usefixtures("cleanup_collection")
    def test_drop_collection_timeout_kwarg(self, cb_env):
        scope_name = '_default'
        collection_name = self.TEST_COLLECTION
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, scope_name, collection_name)
        assert cb_env.get_collection(scope_name, collection_name) is not None
    ...
    def test_drop_collection_collection_spec_kwarg(self, cb_env):
        spec = CollectionSpec(self.TEST_COLLECTION)
        run_in_reactor_thread(cb_env.test_bucket_cm.create_collection, spec)
        assert cb_env.get_collection('_default', self.TEST_COLLECTION) is not None
        run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, collection=spec)
        with pytest.raises(CollectionNotFoundException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, collection=spec)
    ...
    def test_drop_collection_invalid_args(self, cb_env):
        spec = CollectionSpec('coll', 'scope')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, spec, collection=spec)
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, spec, scope_name='_default')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, collection=spec, collection_name='c')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, '_default', collection=spec)
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, '_default', scope_name='_default')
        with pytest.raises(InvalidArgumentException):
            run_in_reactor_thread(cb_env.test_bucket_cm.drop_collection, '_default', 'c', collection_name='c')
    ...
    def test_update_collection_history_retention_unsupported(self, cb_env):
        ...
        run_in_reactor_thread(cm.create_collection, scope_name, collection_name)
        collection_spec = cb_env.get_collection(scope_name, collection_name, bucket_name, cm)
        assert collection_spec is not None
        assert collection_spec.history is False

        # Couchstore does not support history retention
        with pytest.raises(FeatureUnavailableException):
            run_in_reactor_thread(cm.update_collection,
                                  scope_name,
                                  collection_name,
