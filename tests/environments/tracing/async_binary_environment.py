...
class AsyncBinaryTracingEnvironment(AsyncBaseTracingEnvironment):

    async def setup_binary_data(self) -> None:  # noqa: C901
        ...
        for k, v in self.data_provider.get_bytes_docs(num_docs).items():
            for _ in range(3):
                try:
                    _ = await self.collection.upsert(k, v, transcoder=tc)
