...
class AsyncSubdocTestEnvironment(AsyncTestEnvironment):
    ...
    async def load_data(self):  # noqa: C901
        ...
        for k, v in self.data_provider.get_array_only_docs(5).items():
            for _ in range(3):
                try:
                    _ = await self.collection.upsert(k, v)
                    self._loaded_docs[k] = v
                    break
                except (AmbiguousTimeoutException, UnAmbiguousTimeoutException):
                    await asyncio.sleep(3)
        ...
        for k in self._loaded_docs.keys():
            await AsyncTestEnvironment.try_n_times(5, 1, self.collection.get, k)
