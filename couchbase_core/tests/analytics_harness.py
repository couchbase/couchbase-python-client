...
class AnalyticsTestBase(RealServerTestCase):
    ...
    def setUpClass(cls):
        ...
        AnalyticsTestBase.datasets = {
            'beers': [
                'beer', '21A IPA'], 'breweries': [
                'brewery', 'Kona Brewing']}

    def setUp(self, **kwargs):
        if not os.environ.get("PYCBC_USE_ANALYTICS"):
            raise SkipTest("Skipping broken analytics tests")
        pycbc_min_analytics = version_to_tuple(
            os.environ.get("PYCBC_MIN_ANALYTICS"))
        ...
        logging.error(
            "Testing against server version {}".format(PYCBC_SERVER_VERSION))
        ...
        if not self.is_mock:
            if not AnalyticsTestBase.initialised:
                ...
                for dataset, source in AnalyticsTestBase.datasets.items():
                    self.perform_query(
                        "CREATE DATASET {} ON `beer-sample` WHERE `type` = '{}';".format(
                            dataset, source[0]),
                        quiet=True)
    ...
    def cleanUp(self):
        if not self.is_mock:
            ...
            for dataset, source in AnalyticsTestBase.datasets.items():
                self.perform_query(
                    "DROP DATASET {} IF EXISTS;".format(dataset), quiet=True)
            time.sleep(10)
        AnalyticsTestBase.initialised = False
    ...
    def initiate_query(self, statement, options, *args, **kwargs):
        logging.info(
            "initiating query {} : {}, {}".format(
                statement, args, kwargs))
        query = couchbase_core.analytics.AnalyticsQuery(
            statement, *args, **kwargs)
...
def build_expected_query_opts(expected_query, kwarg_type, prefix=""):
    ...
    if len(kwargs):
        expected_query.update(
            {prefix + "{}".format(k): v for k, v in kwargs.items()})
...
class AnalyticsTestQueriesBase(AnalyticsTestBase):
    ...
    def _check_response(self, encoded, query_file, result, responsedict):
        ...
        self.assertSanitizedEqual(
            result, expected, {
                u'meta': 'cas', 'Dataverse': 'Timestamp'})
...
class AnalyticsTestQueries(AnalyticsTestQueriesBase):
    ...
    def test_query(self, query_file):
        ...
        args, kwargs, statement, options = self.gen_query_params(
            query_file, cbas_response)
        result, encoded = self.perform_query_with_options(
            statement, options, *args, **kwargs)
        if gen_reference:
            decoded_encoded = json.loads(encoded)
...
class DeferredAnalyticsTest(AnalyticsTestQueriesBase):
    ...
    def _handles(self):
        if not DeferredAnalyticsTest._responses:
            ...
            for query_file in cbas_response.keys():
                ...
                args, kwargs, statement, options = self.gen_query_params(
                    query_file, cbas_response)
                real_statement = couchbase_core.analytics.DeferredAnalyticsQuery(
                    statement, *args, **kwargs)
                ...
                logging.error(
                    "scheduling query {} with args{} kwargs {} and options {}".format(real_statement, args, kwargs,
                                                                                      options))
                logging.error(
                    "query content; {}, body: {}".format(
                        real_statement, real_statement._body))
                deferred_query = self.do_analytics_query(real_statement)
                logging.error(
                    "scheduled query {}, got response {}".format(
                        real_statement, deferred_query))
    ...
    def test_deferred(self):
        ...
        for query_file, response in self._handles.items():
            ...
            logging.error(
                "checking response for {}: {}".format(
                    query_file, result))
            try:
                self._check_response(
                    encoded, query_file, result, cbas_response)
            except Exception as e:
    ...
    def test_correct_timeout_in_constructor(self):
        ...
        def creator(query, timeout): return couchbase_core.analytics.DeferredAnalyticsRequest(query,
                                                                                              self.get_fixture(),
                                                                                              timeout=timeout)
        self._check_finish_time_in_bounds(x, creator, 500)

    def _check_finish_time_in_bounds(
            self, query, response_creator, expected_timeout):
        orig_time = time.time()
